...
class BIN_Interaction_Flat(nn.Sequential):
    ...
    def __init__(self, **config):
        ...
        self.d_encoder = Encoder_MultipleLayers(self.n_layer, self.hidden_size, self.intermediate_size,
                                                self.num_attention_heads, self.attention_probs_dropout_prob, self.hidden_dropout_prob)
    ...
    def forward(self, d, p, d_mask, p_mask):
        ...
        # Interaction tensor
        i = d_aug * p_aug
        print(f"Interaction tensor shape (i): {i.shape}")
        
        i_v = i.view(int(self.batch_size / self.gpus), -1, self.max_d, self.max_p)
        print(f"i_v after view: {i_v.shape}")
        
        i_v = torch.sum(i_v, dim=1)
        print(f"i_v after sum: {i_v.shape}")
        
        i_v = torch.unsqueeze(i_v, 1)
        print(f"i_v after unsqueeze: {i_v.shape}")
        
        i_v = F.dropout(i_v, p=self.dropout_rate)
        print(f"i_v after dropout: {i_v.shape}")
        
        # Interaction CNN
        f = self.icnn(i_v)
        print(f"f after CNN: {f.shape}")
        
        f = f.view(int(self.batch_size / self.gpus), -1)
        print(f"f after view: {f.shape}")
