...
class BIN_Interaction_Flat(nn.Module):
    ...
    def __init__(self, **config):
        ...
        # Flatten dimension for the decoder
        self.flatten_dim = 2 * self.emb_size
    ...
    def forward(self, d, p):
        # Input dimensions
        print(f"Input drug_emb shape: {d.shape}")
        print(f"Input protein_emb shape: {p.shape}")

        # Gated Cross-Attention
        V_d_prime, V_p_prime = self.gca(d, p)
        print(f"V_d_prime shape: {V_d_prime.shape}")
        print(f"V_p_prime shape: {V_p_prime.shape}")

        # Pooling operation
        d_pooled = torch.mean(V_d_prime, dim=1)
